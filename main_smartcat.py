...
Fallback chain (tried in order, stops at first success):
  1. Veo3 Browser      — Google Flow, Veo3.1 Fast (primary)
  2. Omni Flash        — Google Flow, Omni Flash model, 30cr/10s (same quota as Veo3)
  3. Gemini Web        — gemini.google.com, Omni model, subscription-based
  4. Kling AI          — klingai.com, daily free credits, watermark  [commented out]
  5. OiiOii            — Seedance 2.0 Pro, STAR tokens required       [commented out]
  6. HuggingFace Wan2.1 — fully free, no watermark (queue wait ~8-12 min)
...
OMNI_BROWSER_SCRIPT    = BASE_DIR / "omni_browser.py"
GEMINI_VIDEO_SCRIPT    = BASE_DIR / "gemini_video_browser.py"
...
def _generate_omni(prompt: str, output_path: Path) -> bool:
    """Google Flow / Omni Flash — same quota as Veo3, 30cr/10s (~33 videos/1000cr)."""
    return _run_browser_script(
        OMNI_BROWSER_SCRIPT, prompt, output_path,
        label="OmniFlash", timeout=600,
    )


def _generate_gemini_web(prompt: str, output_path: Path) -> bool:
    """Gemini Web (gemini.google.com) — Omni model, subscription-based, no extra cost."""
    return _run_browser_script(
        GEMINI_VIDEO_SCRIPT, prompt, output_path,
        label="GeminiWeb", timeout=600,
    )
...
def generate_video(prompt: str, output_path: Path,
    ...
    Chain order:
      Veo3 → OmniFlash → GeminiWeb → [Kling] → [OiiOii] → HuggingFace Wan2.1
    ...
    # 2. Omni Flash — same platform as Veo3, different model, 30cr/10s
    log.warning("Veo3 failed, trying Omni Flash...")
    if _generate_omni(prompt, output_path):
        return True, "omni"

    # 3. Gemini Web — gemini.google.com Omni, subscription-based
    log.warning("Omni Flash failed, trying Gemini Web...")
    if _generate_gemini_web(prompt, output_path):
        return True, "gemini_web"

    # 4. Kling AI — uncomment to enable
    # log.warning("Gemini Web failed, trying Kling AI...")
    # if _generate_kling(prompt, output_path):
    #     return True, "kling"

    # 5. OiiOii — uncomment to enable
    # log.warning("Kling failed, trying OiiOii...")
    # if _generate_oiioii(prompt, output_path):
    #     return True, "oiioii"

    # 6. HuggingFace Wan2.1 (final fallback, always free)
    log.warning("Gemini Web failed, falling back to HuggingFace Wan2.1...")
...
BACKEND_LABELS = {
    "veo3":        "Veo3 (Google Flow, Veo3.1 Fast, no watermark)",
    "omni":        "Omni Flash (Google Flow, 30cr/10s, no watermark)",
    "gemini_web":  "Gemini Web (gemini.google.com, Omni, subscription)",
